<div class="card-body p-4">
    ...
    <div class="row">
        ...
        <div class="row">
            ...
            <div class="col-md-2 mt-3">
                {!! Form::select('gerar_conta_receber','Conta/receber ?', [0 => 'Não', 1 => 'Sim'])
                ->attrs(['class' => 'form-select']) !!}
            </div>
            <div class="col-md-2 mt-3">
    ...
    <b><label for="inp-cliente_id" class="">Cliente Gimape</label></b>
    <div class="d-flex gap-2 align-items-center">
        <div class="flex-grow-1">
            <select class="form-control select2 cliente_id" name="cliente_id" id="inp-cliente_id">
                @isset($item)
                    <option value="{{ $item->cliente_id }}">{{ $item->nomeCliente }} - {{ $item->documento }}</option>
                @endisset
            </select>
             <br>
        </div>
        ...
        <div>
    <!--button id="btn-buscar-pedido" type="button" class="btn btn-info px-3" data-bs-toggle="modal" data-bs-target="#modal-pedido" title="Buscar Pedido">
...
modalPedido.addEventListener('show.bs.modal', function (event) {
  ...
  var clienteId = button.getAttribute('data-idcliente')

  if (clienteId) {
    // Exemplo: preencher campo hidden ou input com o id
    var inputCliente = modalPedido.querySelector('#clientePedido')
    inputCliente.value = '' // limpa o campo

    // Busca os dados do cliente via AJAX (exemplo usando jQuery)
    $.ajax({
      url: '/api/clientes/' + clienteId,  // Ajuste para sua rota API
      method: 'GET',
      success: function(data) {
        // Supondo que o retorno seja um objeto com nomeCliente
        inputCliente.value = data.nomeCliente || ''
        // Você pode preencher outros campos aqui também
      },
      error: function() {
