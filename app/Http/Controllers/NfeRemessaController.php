    ...
    try {
        $result = DB::transaction(function () use ($request, $numero_sequencial) {
            ...
            for ($i = 0; $i < sizeof($request->produto_id); $i++) {
                $totalProdutos += __convert_value_bd($request->sub_total[$i]);
            }

            // 🔹 Normaliza descontos, acréscimos e frete
            ...
            $frete     = $request->valor_frete ? __convert_value_bd($request->valor_frete) : 0;

            // 🔹 Fórmula oficial SEFAZ para vNF (Valor Total da Nota)
            $valor_total = $totalProdutos - $desconto + $acrescimo + $frete;

            $dataNFe = [
                'cliente_id' => $request->cliente_id,
                'empresa_id' => $request->empresa_id,
                'usuario_id' => get_id_user(),
                'valor_total' => $valor_total, // ✅ Corrigido
                'forma_pagamento' => $request->tipo_pagamento,
                'numero_nfe' => 0,
                'natureza_id' => $request->natureza_id,
                'chave' => '',
                'estado_emissao' => 'novo',
                'observacao' => $request->informacao ?? '',
                'desconto' => $desconto,
            ...
            if ($request->valor_parcela) {
                for ($i = 0; $i < sizeof($request->valor_parcela); $i++) {
                    $data_vencimento = $request->data_vencimento[$i] ?? date('Y-m-d');
