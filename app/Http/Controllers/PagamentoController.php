...
{
    // Verifica se o planoEmpresa foi fornecido corretamente
    if ($planoEmpresa) {
        // Recupera o plano associado ao planoEmpresa
        $plano = $planoEmpresa->plano; // Supondo que plano_empresa tenha o relacionamento com o modelo Plano

        // Verifica o campo intervalo_dias, que define o número de dias de validade
        $intervaloDias = $plano->intervalo_dias ?? 30; // Se não houver valor, define como 30 dias como padrão

        // Define a nova data de expiração
        $novaExpiracao = now()->addDays($intervaloDias)->format('Y-m-d'); // Formata para 'YYYY-MM-DD'

        // Atualiza a expiração no modelo PlanoEmpresa
        $planoEmpresa->expiracao = $novaExpiracao;
        $planoEmpresa->save(); // Salva o modelo com a nova data de expiração

        // Definindo uma mensagem flash de sucesso
        session()->flash('flash_sucesso', 'Licença renovada com sucesso!');

        // Redirecionando para a página inicial
        
        return redirect()->back();
        //return redirect()->route('main'); // Altere 'home' para a rota desejada
    }

    // Caso o planoEmpresa não seja fornecido corretamente
    session()->flash('flash_erro', 'Erro ao renovar licença!');
    return redirect()->back();
    //return redirect()->route('login'); // Redireciona para a página inicial em caso de erro
}
    ...
    /*public function detalhesPagamento($code){
        ...
        if($payStatus->status != $payment->status){
            ...
            //var_dump($payment->plano);
           // exit;
            
            $this->setarLicenca($payment->plano->plano_id);
        }
    ...
    public function detalhesPagamento($code)
{
    $payment = Payment::where('transacao_id', $code)->first();
    ...
    if($payStatus->status != $payment->status){
        // Agora você deve buscar o objeto PlanoEmpresa usando o ID do plano
        $planoEmpresa = PlanoEmpresa::find($payment->plano_id); // Altere aqui para o ID correto

        // Chame o método passando o objeto completo PlanoEmpresa
